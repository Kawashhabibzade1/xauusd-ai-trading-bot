...
def build_prediction_frame(validation_df: pd.DataFrame, config: dict[str, Any]) -> pd.DataFrame:
    ...
    frame["session_name"] = frame["hour"].fillna(0).astype(int).map(session_name_from_hour)
    frame["session_allowed"] = frame["session_name"].isin(allowed).astype(int)
    ...
    session_score = np.where(
        frame["session_name"].eq("Overlap"),
        1.0,
        np.where(frame["session_name"].eq("London"), 0.80, np.where(frame["session_name"].eq("New York"), 0.72, 0.0)),
    )
    frame["setup_score"] = (
    ...
    frame["time_of_day_bias"] = np.where(
        frame["session_name"].eq("Overlap"),
        0.80,
        np.where(frame["session_name"].eq("London"), 0.55, np.where(frame["session_name"].eq("New York"), 0.45, -0.15)),
    )
    ...
    no_session = np.where(frame["session_allowed"].eq(0), "SESSION_BLOCKED", "")
...
def build_report(
    ...
    baseline_horizons = {
        ...
        "15": {
            ...
            "trade_precision": float(live_report.get("confidence", {}).get("selected_accuracy", 0.0)),
            "trade_recall": None,
            "overall_log_loss": None,
            "overall_brier": None,
            "confidence_bins": [],
